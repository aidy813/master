...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a VeriCoin address"));
    sendCoinsAction->setCheckable(true);
    sendCoinsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_2));
    tabGroup->addAction(sendCoinsAction);

    sendBitCoinsAction = new QAction(QIcon(":/icons/veriBit"), tr("&VeriBit"), this);
    sendBitCoinsAction->setToolTip(tr("Send BitCoins to a BitCoin address"));
...
void BitcoinGUI::createToolBars()
{
    ...
    toolbar2->addAction(sendBitCoinsAction);
}
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    if (nLastCoinStakeSearchInterval && nWeight)
    {
        ...
        labelStakingIcon->setToolTip(tr("Staking.<br>Network interest rate is: %1%<br>Network stake is: %2<br>Expected time to earn interest is: %3").arg(nInterestRate).arg(nNetworkWeight).arg(text));
    }
    else
    {
